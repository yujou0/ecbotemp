...
<script lang="ts" setup>
  ...
  updateDpDateHoverEndCount();

    });
    const dpDateHoverEndCount = ref(0); // 初始化类名为.dp__date_hover_end的元素数量为0


const updateDpDateHoverEndCount = () => {
  const dpDateHoverEndElements = document.querySelectorAll('.dp__date_hover_end');
  dpDateHoverEndCount.value = dpDateHoverEndElements.length;
};

// 根据数量判断是否允许添加类名
const canAddDpDateHoverEnd = computed(() => {
  return dpDateHoverEndCount.value < 10;
});
